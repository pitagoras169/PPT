...
int main(void){
	...
	// Welcome message
	std::cout << "RPS Batch : Markov Vs Second Guessing. \n";
	...
	for (iter = 0 ; iter < max_nb_games ; iter++){
		...
		user_choice = get_second_guessing_choice(user_choice_previous);
	...
	//Save log files
	save_log_file_batch (user_choice_history,machine_choice_history);
	//Statistics of the games and exit message
	display_statistics_batch(nb_games_user_won,nb_games_machine_won,nb_games_tie);
